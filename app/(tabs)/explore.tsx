...
import {Keyboard, ActivityIndicator, StyleSheet, View, Alert, Button, Animated, Pressable, Text, FlatList, Image, TextInput} from 'react-native';
import {db, bucket, supabaseAnonKey, supabaseUrl} from ',,/../components/db/db'
import { MaterialIcons } from '@expo/vector-icons';
import type {Pin, Cat} from 'constants/DataTypes';
...
export default function App() {
  ...
  console.log('data', data)
   ...
   const fetchSignedImageUrl = async (catId: number, file_path: string) => {
    try {
       const res = await fetch(bucketDownloadURL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json',
          'Authorization': `Bearer ${supabaseAnonKey}`
         },
        body: JSON.stringify({ file_path, bucket }),
      });

      const { signedUrl } = await res.json();
      console.log('signedUrl', signedUrl)
      setImageUrls(prev => ({ ...prev, [catId]: signedUrl }));
    } catch (err) {
...
const CatItem = ({ cat }: { cat: Cat}) => (
  <View style={{ marginBottom: 20 }}>
    <Text>{cat.name} - {cat.description}</Text>
    {imageUrls[cat.id] ? (
      <Image
  ...
  <View style={[styles.buttonContainer, addingCat ? styles.buttonBad : styles.buttonGood]}>
       ...
       {addingCat && <View style={[styles.buttonContainer2, styles.buttonConfirm]}>
        {addingCat && !confirmLocation && <Button
      ...
      <Button title="Submit" onPress={submitForm} />
        ...
        <FlatList 
          style={{display: addingCat ? 'none': 'flex'}}
              ...
              data={cats}  renderItem={({ item }) => <CatItem cat={item}/>}/>
      </Animated.View>
    </View>
    </View>
  );
